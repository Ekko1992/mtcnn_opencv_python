...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    for scale in scales:
        ...
        #blob = dnn.blobFromImage(img,size=(hs,ws),mean=(127.5,127.5,127.5),scalefactor=0.0078125)
        #print blob
        PNet.setInput(im_data,"data")

        #outBlobNames = ["conv4-2","prob1"]
        out_conv42 = PNet.forward("conv4-2")
        out_prob1 = PNet.forward("prob1")
        #print "pnet output:"

        boxes = generateBoundingBox(out_prob1[0,1,:,:],out_conv42[0], scale, threshold[0])
        #print boxes
        if boxes.shape[0] != 0:
            #print(boxes[4:9])
    ...
    if numbox > 0:
        ...
        RNet.setInput(tempimg,"data")  
        Rout_conv52 = RNet.forward("conv5-2")
        Rout_prob1 = RNet.forward("prob1")
        ...
        if numbox > 0:
            ...
            # ONet
            tempimg = np.swapaxes(tempimg, 1, 3)
            ONet.setInput(tempimg)
            Oout_con62 = ONet.forward('conv6-2')
            Oout_con63 = ONet.forward('conv6-3')
            Oout_prob1 = ONet.forward('prob1')
            #print Oout_prob1
